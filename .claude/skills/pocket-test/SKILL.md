...
`/api/status`, `/api/files`, `/download`, `/delete`, `/mkdir` and the WebSocket upload handshake. It
rejects frames over 2048 bytes exactly as the firmware does, and reports a `uuid` so identity
resolution takes the stable path.
...
`tests/acceptance.sh` walks the whole product in throwaway dirs under `$TMP`: ingest 5 formats →
build a list → bind an X4 profile → sync → `add_new` → `mirror` delete → mid-transfer drop → resume
→ duplicate protection. It also unpacks the EPUBs that landed on the device and asserts they are
device-safe:
